...
//    to the vertices
int Index = 0;
void quad( int a, int b, int c, int d, const point3& normal, int type )
{
    ...
    else if(type == 1)
    {
        ...
        cubeUV[Index] = point2(-0.5f, 1.5f); Index++;
        cubePoints[Index] = vertices[b]; cubeNormals[Index] = normal;
        cubeUV[Index] = point2(-0.5f, -0.5f); Index++;
        cubePoints[Index] = vertices[c]; cubeNormals[Index] = normal;
        cubeUV[Index] = point2(1.5f, -0.5f); Index++;
        cubePoints[Index] = vertices[a]; cubeNormals[Index] = normal;
        cubeUV[Index] = point2(-0.5f, 1.5f); Index++;
        cubePoints[Index] = vertices[c]; cubeNormals[Index] = normal;
        cubeUV[Index] = point2(1.5f, -0.5f); Index++;
        cubePoints[Index] = vertices[d]; cubeNormals[Index] = normal;
        cubeUV[Index] = point2(1.5f, 1.5f); Index++;
    }
...
void colorcube(int type)
{
    quad( 1, 0, 3, 2, point3( 0.0f,  0.0f,  1.0f), type );
    quad( 2, 3, 7, 6, point3( 1.0f,  0.0f,  0.0f), type );
    quad( 3, 0, 4, 7, point3( 0.0f, -1.0f,  0.0f), type );
    quad( 6, 5, 1, 2, point3( 0.0f,  1.0f,  0.0f), type );
    quad( 4, 5, 6, 7, point3( 0.0f,  0.0f, -1.0f), type );
    quad( 5, 4, 0, 1, point3(-1.0f,  0.0f,  0.0f), type );
}

// initialization
void generateCube(GLuint program, ShapeData* cubeData, int type)
{
    colorcube(type);
